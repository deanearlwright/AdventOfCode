...
//                             a o c _ 0 3 . t s
//
// Solve the Advent of Code 2016 day 03 problem
// ======================================================================
...
function parseCommandLine(): aocArguments {
  ...
  const { argv } = yargs
    .command('aoc_03', 'Squares With Three Sides - Day 03 of Advent of Code 2016', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: aocArguments, inputLines: string[]): boolean {
  ...
  const solver = new Triangles(inputLines, false);
...
function partTwo(args: aocArguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Triangles(inputLines, true);
