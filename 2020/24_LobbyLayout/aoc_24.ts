...
//                             a o c _ 2 4 . t s
//
// Solve the Advent of Code 2020 day 24 problem
// ======================================================================
...
function parseCommandLine(): Arguments {
  ...
  const { argv } = yargs
    .command('aoc_24', 'Lobby Layout - Day 24 of Advent of Code 2020', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: Arguments, inputLines: string[]): boolean {
  ...
  const solver = new Tiles(inputLines, false);
...
function partTwo(args: Arguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Tiles(inputLines, true);
...
function main() {
  ...
  if (argv.part === 1) {
    result = partOne(argv, inputText); // 263
  } else {
    result = partTwo(argv, inputText); // 3649
  }
