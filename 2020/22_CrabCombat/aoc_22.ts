...
//                             a o c _ 2 2 . t s
//
// Solve the Advent of Code 2020 day 22 problem
// ======================================================================
...
function parseCommandLine(): Arguments {
  ...
  const { argv } = yargs
    .command('aoc_22', 'Crab Combat - Day 22 of Advent of Code 2020', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: Arguments, inputLines: string[]): boolean {
  ...
  const solver = new Game(inputLines, false);
...
function partTwo(args: Arguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Game(inputLines, true);
...
function main() {
  ...
  if (argv.part === 1) {
    result = partOne(argv, inputText); // 32629
  } else {
    result = partTwo(argv, inputText); // 35196
  }
