...
//                             a o c _ 1 6 . t s
//
// Solve the Advent of Code 2020 day 16 problem
// ======================================================================
...
function parseCommandLine(): Arguments {
  ...
  const { argv } = yargs
    .command('aoc_16', 'Ticket Translation - Day 16 of Advent of Code 2020', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: Arguments, inputLines: string[]): boolean {
  ...
  const solver = new Tickets(inputLines, false);
...
function partTwo(args: Arguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Tickets(inputLines, true);
...
function main() {
  ...
  if (argv.part === 1) {
    result = partOne(argv, inputText); // 26869
  } else {
    result = partTwo(argv, inputText); // 855275529001
  }
