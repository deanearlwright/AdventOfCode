...
//                             a o c _ 1 8 . t s
//
// Solve the Advent of Code 2020 day 18 problem
// ======================================================================
...
function parseCommandLine(): Arguments {
  ...
  const { argv } = yargs
    .command('aoc_18', 'Operation Order - Day 18 of Advent of Code 2020', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: Arguments, inputLines: string[]): boolean {
  ...
  const solver = new Homework(inputLines, false);
...
function partTwo(args: Arguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Homework(inputLines, true);
...
function main() {
  ...
  if (argv.part === 1) {
    result = partOne(argv, inputText); // 650217205854
  } else {
    result = partTwo(argv, inputText); // 20394514442037
  }
