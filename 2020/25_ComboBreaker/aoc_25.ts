...
//                             a o c _ 2 5 . t s
//
// Solve the Advent of Code 2020 day 25 problem
// ======================================================================
...
function parseCommandLine(): Arguments {
  ...
  const { argv } = yargs
    .command('aoc_25', 'Combo Breaker - Day 25 of Advent of Code 2020', { })
    .option('verbose', {
      alias: 'v',
      default: false,
...
function partOne(args: Arguments, inputLines: string[]): boolean {
  ...
  const solver = new Handshake(inputLines, false);
...
function partTwo(args: Arguments, inputLines: string[]): boolean {
  ...
  // 1. Create the puzzle solver
  const solver = new Handshake(inputLines, true);
...
function main() {
  ...
  if (argv.part === 1) {
    result = partOne(argv, inputText); // 9620012
  } else {
    result = partTwo(argv, inputText); // Merry Christmas to all and to all a Good Night
  }
