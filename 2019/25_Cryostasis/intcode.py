...
#                           i n t c o m p  . p y
# ======================================================================
"Computer for Sensor Boost problem for Advent of Code 2018 Day 09"
...
class IntCode():
    ...
    def __init__(self, # pylint: disable=R0913
                 ...
                 relative=None, default_inp=None):
    ...
    def fetch(self, which):
        ...
        if which < 0:
            return None
        if which >= len(self.positions):
            self.positions = self.positions + [0]*(100 + which - len(self.positions))
    ...
    def operands(self, modes, store):
        ...
        # There are two parameter mode...modes. We'll call these interpreted and literal.
        # Given ram, the intcode array, raw, the parameter value, and mode, the parameter mode:
        ...
        #   When resolving the third parameter for writes,
        #      for example opcodes 1, 2, 7, 8...
        #   When resolving the first parameter for writes,
        #      for example opcode 3 (this is the 203 error)
        ...
        for num, mode in enumerate(modes):
            ...
            if operand is not None:
                if mode == POS_MODE and num != store:
                    operand = self.fetch(operand)
                elif mode == REL_MODE and num != store:
                    operand = self.fetch(self.rbase + operand)
                elif mode == REL_MODE and num == store:
                    operand = self.rbase + operand
    ...
    def execute(self, opcode, ops, watch):
        ...
        # 9. Or Store 1/0 if equal
        elif opcode == OP_REL:
            result = self.execute_rel(ops, watch)
        # 10. Return the result (STOP_RUN or Error)
        return result
    ...
    def execute_input(self, ops, watch):
        ...
        # 3. If there is a default input, use it
        elif self.default_inp is not None:
            value = self.default_inp

        # 4. If no input, set up to redo INP, and stop
        else:
            if watch:
                print("INP - no input - stopping", end=' ')
            self.counter -= 2
            return STOP_INP
        ...
        # 6. Store the input value
        if self.alter(ops[0], value) is None:
            result = STOP_STR
            if watch:
                print("Bad store", end=' ')

        # 7. Return the result (STOP_RUN or Error)
        return result
    ...
    def execute_rel(self, ops, watch):
        "Execute a set relative base instruction"

        # 1. Assume all be well
        result = STOP_RUN

        # 2. Conpute the value
        new_rbase = self.rbase + ops[0]

        # 3. Describe the operation
        if watch:
            print("REL relative base new %d = old %d + op %d" %
                  (new_rbase, self.rbase, ops[0]), end=' ')

        # 4. Store the new relative base
    ...
    def reload(self, text=None):
        "Reload the computer memory"

        # 1. First reset the machine
        self.reset()
    ...
    def any_inp(self):
        "Return True if there is input waiting for the computer"
