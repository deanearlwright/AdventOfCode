...
class Droid():
    ...
    def __init__(self, origin=(0, 0), text=None):
        ...
        self.ship = ship.Ship(start=self.origin)
        self.loc = origin
    ...
    def run(self, watch=False):
        ...
        while result == intcode.STOP_INP and self.state != STATE_FOUND:
            ...
            result = self.computer.run(watch=watch, inp=[self.next_cmd])
    ...
    def oxygen_path(self):
        ...
        while loc != self.origin:
            ...
            rdir = self.ship.go_back(self.loc)
            delta = DELTA[rdir]
            loc = (loc[0] + delta[0], loc[1], delta[1])

            # 6. Record the droid move
            result.append(REVERSE[rdir])
