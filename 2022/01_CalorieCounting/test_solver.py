...
# ======================================================================
"Test solver for Advent of Code 2022 day 01, Calorie Counting"
...
PART_ONE_RESULT = 24000
PART_TWO_RESULT = 45000
...
class TestSolver(unittest.TestCase):  # pylint: disable=R0904
    ...
    def test_empty_init(self):
        ...
        self.assertEqual(myobj.most, 0)
        self.assertEqual(len(myobj.each), 0)

    def test_text_init(self):
        "Test the Solver object creation from text"

        # 1. Create Solver object from text
        myobj = solver.Solver(text=aoc_01.from_text(EXAMPLE_TEXT))

        # 2. Make sure it has the expected values
        self.assertEqual(myobj.part2, False)
        self.assertEqual(len(myobj.text), 15)
        self.assertEqual(myobj.most, 24000)
        self.assertEqual(len(myobj.each), 5)

    def test_part_one(self):
        "Test part one example of Solver object"

        # 1. Create Solver object from text
        myobj = solver.Solver(text=aoc_01.from_text(PART_ONE_TEXT))
    ...
    def test_part_two(self):
        ...
        # 1. Create Solver object from text
        myobj = solver.Solver(part2=True, text=aoc_01.from_text(PART_TWO_TEXT))
