...
#                           a o c _ 2 4 . p y
# ======================================================================
"Solve the puzzles for Advent of Code 2015 day 24"
...
def parse_command_line():
    ...
    desc = 'It Hangs in the Balance - Day 24 of Advent of Code 2015'
    sample = 'sample: python aoc_24.py input.txt'
...
def part_one(args, input_lines):
    ...
    solver = packages.Packages(part2=False, text=input_lines)

    # 2. Determine the solution for part one
    solution = solver.part_one(verbose=args.verbose, limit=args.limit)  # pylint: disable=assignment-from-none
    if solution is None:
...
def part_two(args, input_lines):
    ...
    # 1. Create the puzzle solver
    solver = packages.Packages(part2=True, text=input_lines)

    # 2. Determine the solution for part two
    solution = solver.part_two(verbose=args.verbose, limit=args.limit)  # pylint: disable=assignment-from-none
    if solution is None:
        print("There is no solution")
    else:
...
def main():
    ...
    if args.part == 1:
        result = part_one(args, input_text)  # 11266889531
    else:
        result = part_two(args, input_text)  # 77387711
